...
const COLS = [
    { label: 'Product Name',initialWidth: 170, fieldName: 'purl', type: 'url', typeAttributes: { label: { fieldName: 'Name' } } },
    { label: 'Product Code',initialWidth: 180, fieldName: 'ProductCode', type: 'text' },
    { label: 'Unit Price',initialWidth: 170, fieldName: 'Price', type: 'currency' },
    { label: 'Product Description',initialWidth: 170, fieldName: 'Description', type: 'text' },
    { label: 'Product Family',initialWidth: 200, fieldName: 'Family', type: 'text' }
];
export default class AddNewProductCustom extends LightningElement {
    ...
    async connectedCallback() {
        ...
        await findProducts({ recordId: this.recordId, name: this.searchKey, productCode: '', productFamily: [], RecordLimit: 100 }).then(result => {
    ...
    doCheckData() {
        doCheck({ recIds: this.recordId })
            .then(result => {
                ...
                var data = [];
                result.forEach((item) => {
                    data.push(item.Product__r.ProductCode__c);
                })
    ...
    openModal() {
        this.isModalOpen = true;
        findProducts({ recordId: this.recordId, name: this.searchKey, productCode: '', productFamily: [], RecordLimit:100 }).then(result => {
            console.log(result);
            let dataObj = JSON.parse(result);
            console.log(dataObj);
            this.AllProductData = dataObj.productList;
    ...
    nextDetails() {
        ...
        this.SelectedProductData.forEach((item) => {
            this.opportunityProducts.forEach((data) => {
                if (item.ProductCode == data.Product__r.ProductCode__c) {
    ...
    handleback() {
        ...
        this.isSecondPage = false;
        this.mapIdQuantity = '';
        this.mapIdSalesPrice = '';
        this.mapIdDate = '';
       this.mapIdLineDescription = '';
